...
public class ValidationUtil {
	 ...
	 * @author Rex.Tan
	 * @exception CustomException 获取属性值出错时抛出异常
	 */
	...
	public static void validate(Object object, Class<?>... groups) {
		...
		if (object instanceof List) {
			...
			for (int i = 0; i < list.size(); i++) {
				Set<ConstraintViolation<Object>> cvs = groups != null ? ValidationUtil.valid(list.get(i), groups)
						: ValidationUtil.valid(list.get(i));
				if (!cvs.isEmpty()) {
			...
			Set<ConstraintViolation<Object>> cvs = groups != null ? ValidationUtil.valid(object, groups)
					: ValidationUtil.valid(object);
			if (!cvs.isEmpty()) {
				StringBuilder msg = new StringBuilder();
			...
			for (Field field : fields) {
				...
				if (attr.equals(field.getName())) {
					try {
						...
						logger.error("检查数据重复操作时, 获取属性" + attr + "的值出错", e);
